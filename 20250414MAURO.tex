...
\documentclass[12pt,twoside,onecolumn,openright,final]{memoir}
...
\setmainfont{Brill} % set the main body font

% Configuración de página y geometría
\usepackage[paperwidth=165mm,paperheight=245mm,text={120mm,200mm}, headsep=0.5cm,centering]{geometry}

% Paquetes para bibliografía y referencias
\usepackage[style=authoryear, backend=biber]{biblatex}
\addbibresource{referencias.bib}

% Paquetes para mejorar la tipografía
\usepackage{microtype}
\usepackage{ragged2e}
\usepackage{hyphenat}
\hyphenation{es-pe-cial-men-te, com-por-ta-miento}

% Paquetes para tablas y contenido
\usepackage{longtable}
\usepackage{array}
\usepackage{booktabs}

% Paquetes para personalización de títulos y encabezados
\usepackage{titlesec}
\usepackage{titletoc}
\usepackage{fancyhdr}

% Paquetes para notas al pie
\usepackage{bigfoot}
...
% Paquete para hipervínculos (debe ir al final)
\usepackage{hyperref}

%%% Configuración de ToC (tabla de contenidos)
\maxtocdepth{subsection} % incluir subsecciones
\renewcommand{\cftchapterpagefont}{}
\renewcommand{\cftchapterfont}{}     % no bold!

%%% Configuración de numeración de secciones
\maxsecnumdepth{subsubsection} % numerar subsecciones

%%% Configuración de encabezados y pies de página
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE,RO]{\fontsize{9}{}\setmainfont{Brill}\thepage}
\fancyhead[CE]{\fontsize{9}{10}\setmainfont{Brill}\scshape Alejandro Bielsa}
\fancyhead[CO]{\fontsize{9}{10}\setmainfont{Brill}\nouppercase{\leftmark}}
\fancyfoot[CE,CO]{}
\renewcommand{\headrulewidth}{0.8pt}
\renewcommand{\footrulewidth}{0pt}
\renewcommand{\sectionmark}[1]{\markboth{#1}{}}

%%% Personalización del formato de capítulos y secciones
\titleformat{\chapter}[display]
{\setmainfont{Brill}\filcenter\large}{\chaptertitlename\ \thechapter}{12pt}{\LARGE}

\titleformat{\section}
{\filcenter\normalfont\normalsize\bfseries\MakeUppercase\scshape}{\thesection}{0.7em}{}
...
{\normalfont\normalsize\bfseries}{\thesubparagraph}{0.7em}{}
...
\titlespacing*{\subparagraph} {\parindent}{2ex plus 1ex minus .2ex}{1em}

%%% Personalización de notas al pie
\renewcommand{\footnoterule}{%
	\kern 2pt % separación entre texto y filete
	\hrule height 0.5pt width 0.15\columnwidth
	\kern 4.6pt % separación entre filete y nota
}

\makeatletter
\renewcommand*\@makefnmark{~(\@thefnmark)} % número de nota entre paréntesis
\renewcommand*\@makefntext[1]{%
	\noindent\hspace*{6mm}(\@thefnmark)\ #1%
}
\makeatother

%%% Redefinir entorno quote para menos espacio
\makeatletter
\renewenvironment{quote}{%
	\setlength{\topsep}{0pt} % Espacio antes
	\setlength{\partopsep}{0pt} % Espacio extra si hay salto de página
	\setlength{\parsep}{0pt} % Espacio entre párrafos dentro del quote
	\list{}{%
		\rightmargin\leftmargin % Márgenes iguales
	}%
	\item\relax % Sin viñeta
}{%
	\endlist
}
\makeatother

%%% Personalización de contadores y numeración
\renewcommand{\thesubsection}{\thesection.\arabic{subsection}}
\renewcommand{\thesubsubsection}{\thesubsection.\alph{subsubsection})}
\renewcommand{\theparagraph}{\thesubsubsection)\nobreakspace\alph{paragraph}}
...
\textbf{Palabras clave: Campo periodístico; Metaperiodismo; Legitimación; Independencia; Libertad de expresión}
...
\textbf{Keywords: Journalistic field; Meta journalism; Legitimation; Independence; Freedom of expression}
...
Desde la reinstalación del sistema democrático, en 1984, es posible identificar el desarrollo y la consolidación de los libros de periodistas como uno de los géneros predominantes del ámbito editorial \parencite{dediego2012}. Por otra parte, este fenómeno constituye una forma de intervención pública y de jerarquización profesional en el ámbito periodístico \parencite{pereira2013, vommaro2012}.

\textcite{baldoni2014cap} dan cuenta del fenómeno mostrando el proceso de incorporación de la producción de libros de periodistas en el mercado editorial, inicialmente desde las pequeñas editoriales. Luego, a partir de la consolidación de este género, las grandes casas editoriales encontraron, como muestra \textcite{saferstein2016}, un nicho de mercado propicio para la producción de \emph{best-sellers}.
...
Como correlato del conflicto entre el gobierno y el campo periodístico se produjo, simultáneamente, un conflicto de grandes proporciones entre el gobierno y los principales medios de comunicación \parencite{arrueta2013, stefoni2013, gindin2014, ure2014}. Si bien este fenómeno (el conflicto con los medios de comunicación) no ha sido exclusivo del gobierno de Cristina Fernández de Kirchner, lo novedoso del período es lo prolongado del mismo y que se trasladó al interior del colectivo de los periodistas. Ello produjo disputas discursivas y denuncias recíprocas alrededor de la deontología de la actividad y los principios que la deben regir, personalizando en colegas la crítica sobre el quehacer de la profesión, de manera explícita o implícita, como no se veía desde el debate Sarmiento-Alberdi de 1853 en torno a este tipo de problemas\footnote{El carácter de caso paradigmático de confrontación en torno al rol del periodismo en la construcción de la república moderna ha sido revisitado por los trabajos de \textcite{shumway2003, demarco2006, moyano2008, ruiz2014} y por Sábato, Myers y Stuven en \textcite[vol. 1]{altamirano2008}}.
...
Por su parte, Françoise Benhamou \parencite[34-35]{benhamou2015} afirma:
...
Mientras que en los diez años previos al año 2009 se editaron menos de cinco títulos\footnote{En 2007 se publicó \emph{Noticias del poder, buenas y malas artes del periodismo político} de Jorge Halperín. Anteriormente en 2006, Hechos y noticias. Claroscuros de la prensa gráfica en la Argentina, de Eduardo Zukernik \parencite{zukernik2006}, y luego un poco más atrás, en el año 2002, Grandes hermanos. Alianzas y negocios ocultos de los dueños de la información de Eduardo Anguita, para aproximarse a un antecedente del fenómeno.} de esa temática, en el período temporal considerado (entre octubre de 2009 y diciembre de 2015) se publicaron aproximadamente cuarenta, haciendo periodismo sobre periodistas o empresas periodísticas. El primero fue \emph{Diario de guerra. Clarín el gran engaño argentino} de  \textcite{diaz2009}, seguido inmediatamente por \emph{678. La creación de otra realidad}, de María Julia Oliván y Pablo Alabarces \parencite{olivan2010}, los que dan inicio a la saga que, a los efectos del presente trabajo, cerraremos provisoriamente en diciembre de 2015 con \emph{Periodistas deportivos} de Walter Vargas.
...
\begin{longtable}{|l|*{9}{c|}} \hline
	\endfirsthead
	\hline
	\endhead
	\hline
	\endfoot
	\caption{\textbf{Evolución de la distribución de la inversión publicitaria por medio entre 2010 y 2018 en Argentina}} \\
	\hline
	\multirow{}{}{Medio} & \multicolumn{9}{c|}{Inversión publicitaria según medios, en \%} \\ \cline{2-10}
	& 2010 & 2011 & 2012 & 2013 & 2014 & 2015 & 2016 & 2017 & 2018 \\ \hline
	\endfirsthead

	Televisión abierta y cable & 47,7 & 44,4 & 45 & 46,9 & 39,5 & 38,6 & 39 & 41 & 41 \\ \hline
	Radios en AMBA e interior & 3,2 & 3,2 & 3,7 & 4,0 & 4,6 & 5 & 8 & 8 & 8 \\ \hline
	Diarios en AMBA e interior & 34 & 35 & 34 & 31 & 25 & 22 & 21 & 16 & 15 \\ \hline
	Revistas & 4,7 & 4,8 & 4,6 & 4,2 & 3,1 & 3 & 3 & 2 & 2 \\ \hline
	Vía pública & 5,5 & 5,2 & 4,3 & 4,3 & 4.5 & 5 & 5 & 5 & 7 \\ \hline
	Cine & 1,3 & 1,2 & 1,3 & 1,3 & 1,2 & 1 & 1 & 1 & 1 \\ \hline
	Internet & 4,3 & 6,1 & 7,4 & 8,0 & 16 & 20 & 20 & 24 & 25 \\ \hline
\end{longtable}
...
\begin{tabular}{|l|*{10}{r@{,}l|}r@{,}l|}

	\hline
	\multicolumn{1}{|c|}{\textbf{Circulación diarios pagos}} &
	\multicolumn{2}{c|}{\textbf{2010}} &
	\multicolumn{2}{c|}{\textbf{2011}} &
	\multicolumn{2}{c|}{\textbf{2012}} &
	\multicolumn{2}{c|}{\textbf{2013}} &
	\multicolumn{2}{c|}{\textbf{2014}} &
	\multicolumn{2}{c|}{\textbf{2015}} &
	\multicolumn{2}{c|}{\textbf{2016}} &
	\multicolumn{2}{c|}{\textbf{2017}} &
	\multicolumn{2}{c|}{\textbf{2018}} &
	\multicolumn{2}{c|}{\textbf{2019}} &
	\multicolumn{2}{c|}{\textbf{Dif. 2010/2019}} \\
	\hline
	Clarín      & 301 & 0 & 288 & 0 & 271 & 0 & 279 & 0 & 239 & 0 & 238 & 0 & 207 & 0 & 183 & 0 & 206 & 0 & 213 & 0 & -33 & 0 \\
	La Nación   & 150 & 0 & 160 & 0 & 162 & 0 & 168 & 0 & 164 & 0 & 156 & 0 & 130 & 0 & 122 & 0 & 117 & 0 & 100 & 0 & -37 & 0 \\
	Diario Popular & 90 & 0 & 91 & 0 & 89 & 0 & 88 & 0 & 82 & 0 & 84 & 0 & 78 & 0 & 69 & 0 & 62 & 0 & 51 & 0 & -48 & 0 \\
	\hline
\end{tabular}
...
\begin{tabular}{|l|l|*{10}{r@{,}l|}|r@{,}l|}
	...
	\multicolumn{2}{c|}{\textbf{2018}} &
	\multicolumn{2}{c|}{\textbf{2019}} &
	\multicolumn{2}{c|}{\textbf{Dif.}} \\
	\hline
	Clarín & AMBA & 664 & 0 & 591 & 0 & 599 & 0 & 570 & 0 & 513 & 0 & 508 & 0 & 487 & 0 & 446 & 0 & 388 & 0 & 396 & 0 & -40 & 0 \\
	La Nación & AMBA & 280 & 0 & 332 & 0 & 360 & 0 & 331 & 0 & 323 & 0 & 320 & 0 & 302 & 0 & 288 & 0 & 209 & 0 & 201 & 0 & -28 & 0 \\
	Diario Popular & AMBA & 147 & 0 & 131 & 0 & 135 & 0 & 131 & 0 & 120 & 0 & 118 & 0 & 106 & 0 & 95 & 0 & 85 & 0 & 73 & 0 & -50 & 0 \\
	La Voz del Int. & Córdoba & 99 & 0 & 90 & 0 & 90 & 0 & 90 & 0 & 84 & 0 & 81 & 0 & 73 & 0 & 66 & 0 & 59 & 0 & 53 & 0 & -46 & 0 \\
	Los Andes & Mendoza & 83 & 0 & 78 & 0 & 77 & 0 & 73 & 0 & 68 & 0 & 65 & 0 & 57 & 0 & 52 & 0 & 49 & 0 & 43 & 0 & -48 & 0 \\
	La Capital & Rosario & 82 & 0 & 80 & 0 & 76 & 0 & 67 & 0 & 62 & 0 & 56 & 0 & 47 & 0 & 41 & 0 & 36 & 0 & 30 & 0 & -63 & 0 \\
	La Gaceta & Tucumán & 61 & 0 & 62 & 0 & 60 & 0 & 56 & 0 & 56 & 0 & 55 & 0 & 49 & 0 & 45 & 0 & 40 & 0 & 34 & 0 & -44 & 0 \\
	Río Negro & Gral. Roca & 45 & 0 & 44 & 0 & 42 & 0 & 36 & 0 & 35 & 0 & 35 & 0 & 32 & 0 & 28 & 0 & 26 & 0 & 21 & 0 & -53 & 0 \\
	El Día & La Plata & 40 & 0 & 41 & 0 & 39 & 0 & 35 & 0 & 35 & 0 & 33 & 0 & 31 & 0 & 26 & 0 & 23 & 0 & 19 & 0 & -52 & 0 \\
	Diario Uno & Entre Ríos & 36 & 0 & 38 & 0 & 22 & 0 & 34 & 0 & 30 & 0 & 28 & 0 & 26 & 0 & 26 & 0 & 25 & 0 & 11 & 0 & -69 & 0 \\
	\hline
\end{tabular}
...
\textbf{Gráfico 2: Distribución de las ediciones por años y editoriales}


PONER GRAFICO oooooooooooooo
...
Es pertinente destacar que el inicio y el fin de las etapas o fases del desarrollo de la prensa operaron de manera asincrónica. Cada una de ellas tuvo solapamientos, hecho que determinó el carácter mixto de la prensa comercial, es decir la articulación de información comercial, social y de interés general con información política y de opinión, beneficiándose cada etapa de la construcción y preparación de las audiencias de las etapas precedentes.
	...
	\begin{table}[h]
	\centering
	\caption{Representación gráfica de los momentos de la prensa \\ según la taxonomía de Jürgen Habermas (1994)}
	\begin{tabular}{|l|c|c|c|c|c|c|}
		\hline
		& \textbf{Siglo XVI} & \textbf{Siglo XVII} & \textbf{Siglo XVIII} & \textbf{Siglo XIX} & \textbf{Siglo XX} & \textbf{Siglo XXI} \\
		\hline
		\textbf{Crematística} & & & & & & \\
		\hline
		\textbf{De opinión} & & & & & & \\
...
\begin{center}
	...
	\begin{longtable}{>{\raggedright\arraybackslash}p{30mm}
			*{8}{>{\centering\arraybackslash}p{11.25mm}}}
		\caption*{Cuadro 5. Condiciones mínimas que reúnen los integrantes del campo de acuerdo a sus posiciones en el polígono de fuerzas} \\
		\hline
		& \small Medios masivos y dominantes
		& \small Medios alternativos
		& \small Firman sus notas
		& \small Altos ingresos económicos
		& \small Grandes audiencias
		& \small Autor de libros
		& \small Presencia en medios audiovisuales
		& \small Objeto de biografías \\
		\hline
		\endfirsthead
		...
		\small Hegemónicos dominantes & Sí & No & Sí & Sí & Sí & Sí & Sí & Sí \\
		\small Hegemónicos alternativos & No & Sí & Sí & Sí/No & Sí/No & Sí & Sí/No & Sí \\
		\small Periféricos dominantes & Sí & No & No & No & Sí & No & No & No \\
		\small Periféricos alternativos & No & Sí & No & No & Sí/No & No & No & No \\
		\hline
...
\paragraph{Los profesionalistas}
...
\paragraph{Los intelectuales periodistas}
...
\begin{document}
	...
	{\small
		\begin{longtable}{@{}>{\RaggedRight}p{4.5cm}>{\RaggedRight}p{3cm}c>{\RaggedRight}p{2.5cm}@{}}
			\caption{Lista de publicaciones}\\
			\toprule
			\textbf{Título y año} & \textbf{Editorial} & \textbf{Nº} & \textbf{Autor} \\
			\midrule
			\endfirsthead
			...
			\multicolumn{4}{l}{\textbf{2011}} \\
			Pecado original & Planeta & 4 & Graciela Mochkofsky \\
			Perón y los medios & Sudam./Penguin & 5 & Pablo Sirvén \\
			K Letra bárbara & Sudam./Penguin & 6 & Orlando Barone \\
			El kirchnerismo póstumo & Ediciones B & 7 & Jorge Asís \\
			Patria o medios & Sudam./Penguin & 8 & Edi Zunino \\
			Periodismo de infantería & Clarinete & 9 & Virginia Márquez y Aníbal Ces \\
			\midrule

			\multicolumn{4}{l}{\textbf{2012}} \\
			Lanata & Margen Izquierdo & 10 & Luis Majul \\
			\midrule

			\multicolumn{4}{l}{\textbf{2013}} \\
			Aquel periodismo (1965-2012) & Edhasa & 11 & Daniel Muchnik \\
			Víctor Hugo: coherencia y convicción & Ed. al arco & 12 & Julián Capasso \\
			El inventor del peronismo & Planeta & 13 & Silvia D. Mercado \\
			Máquinas de captura & Colihue & 14 & Daniel Rosso \\
			Clarín: historia del gran diario & Planeta & 15 & Martín Sivak \\
			...
			\multicolumn{4}{l}{\textbf{2014}} \\
			Nuevos desafíos del periodismo & Ariel/Paidós & 19 & Dessein y Roitberg (Comp.) \\
			Periodistas en el barro & Sudam./Penguin & 20 & Edi Zunino \\
			Tiempos turbulentos & Ariel/Paidós & 21 & Jornet y Dessein \\
			Télam: hecho maldito & Ed. TXT & 22 & Bargach y Suárez \\
			(In)justicia mediática & Sudam./Penguin & 23 & Darío Villarruel \\
			Audiencia con el diablo & Aguilar & 24 & V. H. Morales \\
			Las locuras del Rey Jorge & Ediciones B & 25 & E. Blaustein \\
			Guerras mediáticas & Sudam./Penguin & 26 & Fernando Ruiz \\
			\midrule

			\multicolumn{4}{l}{\textbf{2015}} \\
			Quiénes fuimos en la era k & Planeta & 27 & J. Fontevecchia \\
			El Perro & Ediciones B & 28 & H. López Echague \\
			Ley de medios inconclusa & Continente & 29 & Néstor Piccone \\
			Grandes y pequeñas mentiras & Planeta & 30 & Novaro y Birmajer \\
			Doble agente: Verbitsky & Sudam./Penguin & 31 & G. Levinas \\
			Mentime que me gusta & Aguilar & 32 & V. H. Morales \\
			El rebenque del diablo & Colihue & 33 & V. H. Morales \\
			Todos contra Branca & Ediciones B & 34 & D. Brancatelli \\
			Diez ironías sobre libertad & CTP & 35 & Varios autores \\
			Clarín: la era Magnetto & Planeta & 36 & M. E. Sivak \\
			Periodistas deporivos & Ed. al arco & 37 & Walter Vargas \\
...
\begin{quote}
¿A qué distancia exacta se mantuvo Víctor Hugo Morales de ser colaboracionista o heroico durante la dictadura militar en Uruguay? Esa es la gran pregunta que todos se hacen, solo que algunos la formulan en voz más alta que otros.

Dos expresidentes de la República Oriental del Uruguay, consultados por este libro, \textbf{no recuerdan que Víctor Hugo Morales haya sido heroico —ni mucho menos— durante la dictadura sufrida por ese país entre los años setenta y ochenta.}



		—Que haya sido perseguido, no me consta —dijo el doctor Alberto Lacalle, el dirigente del Partido Blanco que gobernó entre 1990 y 1995—. \textbf{Nunca tuvo una actitud política.} Su familia era del Partido Nacional.

		—Víctor Hugo Morales es un gran periodista, tuvo un notable éxito tanto en Uruguay como en la Argentina y su capacidad es indiscutible. Luego ocurrió que, ya instalado en la Argentina, asumió una actitud como de resistente a la dictadura uruguaya que simplemente no es verdad. Recibió de la dictadura algunos actos muy importantes de apoyo.

		—¿Se refiere al espaldarazo que le dieron los militares cuando los dirigentes del fútbol uruguayo le prohibieron ser relator?

		—Exacto. Es un hecho objetivo. La realidad es esa. Ni tanto ni tan poco. No puedo decir que Víctor Hugo fuera parte de la dictadura ni tampoco claramente todo lo contrario, pero tuvo buenas relaciones con los militares. Eso es un hecho.

		—¿Qué opina de la posición tan fervorosa que ahora tiene sobre el kirchnerismo?

		—Bueno, eso se mira un poco más a la distancia, porque Víctor Hugo no tiene una presencia política en el Uruguay, pero a la gente le ha llamado la atención, sobre todo el entusiasmo con que asume el kirchnerismo.

(Sirvén, 2013: 105).\footnote{El énfasis es propio.}
...
\begin{quote}
No falta mucho para que la Argentina kirchnerista pueda reflejarse en el espejo de la Venezuela chavista, que poco a poco ha ido sustituyendo a la sociedad por el Estado y que exalta un liderazgo personalista incompatible con instituciones y principios republicanos que despersonalizan el poder.

Quizá pocas cosas como la relación entre gobierno y prensa asocien a ambos países. El chavismo ideó un \emph{modus operandi} contra el periodismo independiente cuyos pasos de manual podrían resumirse así:

— Difamar públicamente a propietarios de medios periodísticos críticos del Gobierno para desprestigiarlos y asociar a los periodistas capaces de incomodar al poder político con los supuestos intereses económicos de sus empleadores, apuntando a negarles toda independencia profesional y credibilidad.

— Fortalecer a los medios de comunicación afines y debilitar al resto mediante la asignación arbitraria de publicidad oficial como herramienta para premiar a los complacientes y castigar a los desobedientes.

— Generar nuevos marcos legales para restringir la libertad de prensa y para debilitar económicamente a determinadas empresas periodísticas.

— Estatizar o crear medios bajo la tutela gubernamental a través de empresarios amigos, cuya fidelidad al Gobierno estaría garantizada con jugosas inyecciones de pauta publicitaria oficial.

— Abusar de la cadena nacional para inocular en la población el relato sobre las bondades del modelo oficial y para tildar de conspiradores o golpistas a los medios de prensa críticos del Gobierno.

Una serie de ataques sufridos por la prensa en la era kirchnerista refleja las similitudes entre la Argentina de los últimos años y el régimen fundado por Hugo Chávez:

— En reiteradas ocasiones, la presidenta de la Nación denunció infundadamente a periodistas y medios. El vicepresidente Amado Boudou llegó incluso a comparar a dos periodistas de los diarios \emph{Clarín} y \emph{La Nación} con quienes ``limpiaban las cámaras de gas durante el nazismo''.

— Distintos periodistas críticos del Gobierno fueron dejados cesantes en medios privados para los cuales se desempeñaban por aparentes presiones oficiales (el caso de Marcelo Longobardi en Radio 10 ha sido uno de los más emblemáticos).

— Gremios afines al Gobierno bloquearon plantas impresoras de diarios e impidieron su distribución, en medio de la pasividad de las autoridades.

— Se repitieron maniobras intimidatorias, como la presencia de unos doscientos inspectores de la Administración Federal de Ingresos Públicos (AFIP) en las oficinas de \emph{Clarín} o un embargo contra \emph{La Nación} pese a la existencia de una medida cautelar dispuesta por la Justicia que lo impedía.

— La arbitrariedad y discrecionalidad con que se manejó la asignación de la pauta publicitaria oficial quedó claramente de manifiesto. Medios gráficos cercanos al oficialismo, de escasísima circulación, han llegado a recibir en un año diez veces más pauta de la que reciben del Estado nacional canales de televisión con elevado índice de audiencia\ldots
...
Los autores \textcite[ ]{novaro2015} se corresponden de manera precisa al concepto que \textcite[ 112]{bourdieu2008} asigna al de ``intelectuales periodistas''. En relación con esta categoría señala:
